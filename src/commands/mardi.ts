...
export const mardi: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      if (daysUntilTuesday === 0 && now.getHours() >= 23) {
        // If it's Tuesday but late in the day, get next Tuesday
      ...
      if (events.length === 0) {
        const embed = new EmbedBuilder()
          .setTitle(`📅 Mardi ${displayDate} - Aucun événement`)
          .setDescription('Aucun événement prévu pour ce mardi.')
          .setColor(0xFFA500)
          .setTimestamp()
      ...
      const embed = new EmbedBuilder()
        .setTitle(`📅 Mardi ${displayDate} - ${events.length} événement${events.length > 1 ? 's' : ''}`)
        .setColor(0x4285F4)
        .setTimestamp()
        .setFooter({ text: 'Google Calendar API' });
